...
export default function TopVotes() {
  ...
  useEffect(() => {
    ...
    internetSocket.on("update-votes", handleUpdateVotes);
    internetSocket.on("reset-votes", handleResetVotes);

    return () => {
      internetSocket.off("update-votes", handleUpdateVotes);
      internetSocket.off("reset-votes", handleResetVotes);
    };
  }, []);
